...
//titled "free code" by "SirSpoodles" on the spigot forums slightly modified by OakleyCord
...
public class MessageUtil {
    ...
    public static String generate(String message) {
        return generate(message, 154);
    }


    public static String generate(String message, int centerLength) {
        ...
        final int toCompensate = centerLength - halvedMessageSize;
    ...
    public static String border(List<String> lines) {
        //create a border made up of >,-, and < characters same length as the longest line by pixel size in the list
        StringBuilder border = new StringBuilder();
        int maxPxSize = 0;
        for (String line : lines) {
            int pxSize = getPixelSize(line);
            if (pxSize > maxPxSize) {
                maxPxSize = pxSize;
            }
        }
        ...
        char middle = '-';
        char outer = '<';
        int innerSize = DefaultFontSize.getDefaultFontSize(inner).getLength();
        int middleSize = DefaultFontSize.getDefaultFontSize(middle).getLength();
        int outerSize = DefaultFontSize.getDefaultFontSize(outer).getLength();
    ...
    public static int getPixelSize(String message) {
        int messagePxSize = 0;
